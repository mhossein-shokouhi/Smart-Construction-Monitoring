...
# Optional streaming: (stream_url, camera_id) set from args; queue holds latest JPEG for sender thread
_stream_url = None
_stream_camera_id = 0
_frame_queue = None
_sender_stop = None
...
def get_args():
    ...
    parser.add_argument("--model", type=str, help="Path of the model",
                        default=str(SCRIPT_DIR / "models" / "imx500_network_ssd_mobilenetv2_fpnlite_320x320_pp.rpk"))
...
if __name__ == "__main__":
    ...
    if intrinsics.labels is None:
        with open(SCRIPT_DIR / "assets" / "coco_labels.txt", "r") as f:
            intrinsics.labels = f.read().splitlines()
    ...
    if _stream_url:
        ...
        sender = threading.Thread(target=_stream_sender, daemon=True)
        sender.start()
